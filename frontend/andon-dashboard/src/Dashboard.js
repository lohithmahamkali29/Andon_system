...
//               calltype: calltype,
//               faultTime: safeDateFormat(station.faultTime, formatDateTime),
//               resolvedTime: safeDateFormat(station.resolvedTime, formatDateTime)
//             });
//           }
//         });
//       }
...
//   // Helper to handle null/undefined dates
//   const safeDateFormat = (date, formatter) => {
//     if (!date) return 'N/A';
...
//   // Find the latest fault calltype and resolved time
//   let latestFaultCalltype = null;
//   let latestFaultTime = null;
//   let latestResolvedTime = null;
//   if (station.faults) {
//     station.faults.forEach(fault => {
//       if (!latestFaultTime || new Date(fault.faultTime) > new Date(latestFaultTime)) {
...
//     latestFaultCalltype,
//     latestFaultTime: latestFaultTime ? formatDateTime(latestFaultTime) : null,
//     latestResolvedTime: latestResolvedTime ? formatDateTime(latestResolvedTime) : null,
...
//               </h3>
//               <p style={{ fontSize: '3rem', fontWeight: 'bold', margin: 0 }}>
//                 {Number(currentStation.actualCount || 0).toLocaleString()}
//               </p>
//             </div>
...
//               </h3>
//               <div>
//                 {currentStation.latestFaultCalltype ? (
//                   <>
//                     <div style={{ 
...
//                       {currentStation.faultTime ? formatDateTime(currentStation.faultTime) : '--:--:--'}
//                     </p>
//                   </>
//                 ) : (
...
//               </h3>
//               <div>
//                 {currentStation.latestFaultCalltype && currentStation.resolvedTime ? (
//                   <>
//                     <div style={{ 
//                       display: 'inline-block',
//                       backgroundColor: 'rgba(255,255,255,0.2)',
...
//                     </div>
//                     <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: '0.25rem 0' }}>
//                       {formatDateTime(currentStation.resolvedTime)}
//                     </p>
//                   </>
//                 ) : (
//                   <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>
...
//                         <strong>Fault Time:</strong> {formatDateTime(fault.faultTime)}
//                       </p>
//                       {fault.resolvedTime && (
//                         <p style={{ margin: '0.25rem 0' }}>
//                           <strong>Resolved:</strong> {formatDateTime(fault.resolvedTime)}
//                         </p>
//                       )}
//                       {!fault.resolvedTime && (
...
const Dashboard = () => {
        ...
        Object.entries(station.faultStatus).forEach(([calltype, status]) => {
          if (status === true) {
            faults.push({
              stationName: station.stationName,
              calltype: calltype,
              faultTime: station.latestFaultTime || 'N/A',
              resolvedTime: station.latestResolvedTime || null
            });
          }
        });
      }
  ...
  function mapStationFields(station) {
    ...
    let latestFaultTime = null;
    let latestResolvedTime = null;
    if (station.faults && Array.isArray(station.faults)) {
      station.faults.forEach(fault => {
        if (!latestFaultTime || new Date(fault.faultTime) > new Date(latestFaultTime)) {
          latestFaultCalltype = fault.calltype;
          latestFaultTime = fault.faultTime;
  ...
  function downloadCSV(stationName) {
    ...
    const csvRows = [tableColumns.join(',')];
    filtered.forEach(row => {
      csvRows.push(tableColumns.map(col => {
        const value = row[col] ?? '';
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div>
                {currentStation.latestFaultCalltype && currentStation.latestFaultTime ? (
                  <>
                    <div style={{ 
                      display: 'inline-block',
                      backgroundColor: 'rgba(255,255,255,0.2)',
            ...
            <div style={{
              ...
              <div>
                {currentStation.latestFaultCalltype && currentStation.latestResolvedTime ? (
                  <>
                    <div style={{ 
                      display: 'inline-block',
                      backgroundColor: 'rgba(255,255,255,0.2)',
                    ...
                    <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: '0.25rem 0' }}>
                      {currentStation.latestResolvedTime}
                    </p>
                  </>
                ) : (
                  <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>
      ...
      {showReportsModal && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>Select Station for CSV Export:</label>
              <select
                value={selectedReportStation}
                onChange={(e) => setSelectedReportStation(e.target.value)}
                style={{
            ...
            <button
              onClick={() => selectedReportStation && downloadCSV(selectedReportStation)}
              disabled={!selectedReportStation}
              style={{
                width: '100%',
                backgroundColor: selectedReportStation ? '#3498db' : '#d1d5db',
                color: 'white',
                padding: '0.75rem',
                border: 'none',
                borderRadius: '0.25rem',
                fontSize: '1rem',
                fontWeight: 'bold',
                cursor: selectedReportStation ? 'pointer' : 'not-allowed'
              }}
            >
              Download CSV
            </button>
